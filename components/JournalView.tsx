...
const EntryCard: React.FC<{ entry: JournalEntry }> = ({ entry }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md mb-4 transition-all duration-300">
            <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{entry.entry}</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{new Date(entry.timestamp).toLocaleString()}</p>
            
            {entry.status === 'analyzing' && (
                <div className="mt-4 border-t border-slate-200 dark:border-slate-700 pt-3 flex items-center space-x-2 text-sky-500">
            ...
            {entry.type === 'task' && entry.taskStatus !== 'none' && (
                <div className="mt-4 border-t border-slate-200 dark:border-slate-700 pt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-medium">
...
export const JournalView: React.FC<JournalViewProps> = ({ spreadsheetId, onReset }) => {
                ...
                {error && (
                    <div className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 p-4 rounded-md mb-4">
                        ...
                        {(error.includes("Failed to read the sheet") || error.includes("Spreadsheet not found")) && (
                            <button 
                                onClick={onReset}
                                className="mt-2 px-3 py-1 text-sm font-semibold bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-100 rounded-md hover:bg-red-300 dark:hover:bg-red-700"
                            >
